"""
Random Feature Model (RFM) for Direction Computation

This module implements RFM with metric learning for computing steering directions.
The algorithm iteratively:
1. Fits a kernel ridge regression model with a Laplacian kernel
2. Computes gradients to learn an optimal metric M
3. Extracts the top eigenvector of M as the steering direction

Reference: Radhakrishnan et al., "Mechanism of feature learning in convolutional
neural networks"
"""
...
def rfm(traindata, testdata, L=10, reg=1e-3, num_iters=10, norm=False):
    ...
    Args:
        traindata: Tuple of (X_train, y_train) tensors
        testdata: Tuple of (X_test, y_test) tensors for validation
        L: Kernel bandwidth parameter
        reg: Regularization strength
        num_iters: Number of metric learning iterations
